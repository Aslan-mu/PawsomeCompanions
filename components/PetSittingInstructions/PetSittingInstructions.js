...
export default class PetSittingInstructionsController extends React.Component {
  ...
  componentDidMount = async () => {
    firebaseSvc
      ...
      .where('owner', '==', global.currentUser.id)
      .onSnapshot(querySnapshot => {
        querySnapshot.docChanges().forEach(async change => {
          if (change.type === 'added') {
            ...
            if (
              newSession.startDate.getTime() < dateNow &&
              dateNow < newSession.endDate.getTime()
            ) {
              this.setState({
    ...
    firebaseSvc
      .refPetSittingSessions()
      .where('sitter', '==', global.currentUser.id)
      .onSnapshot(querySnapshot => {
        querySnapshot.docChanges().forEach(async change => {
      ...
      <TabView
        ...
        renderScene={({route}) => {
          switch (route.key) {
            case 'first':
              return (
                <PetSittingInstructions
                  navigation={this.props.navigation}
                  todaySessionData={this.state.todaySession}
                />
              );
            case 'second':
...
const IndividualSessionCard = props => {
  ...
  const [instructions, setInstructions] = React.useState(
    props.sessionData.instructions,
  );
  ...
  const addANewInstruction = ({instruction, repeat, reminder, date}) => {
    ...
    firebaseSvc.addNewInstructionToTheSession(
    ...
    <View style={styles.petSittingCard}>
        ...
        <View>
          ...
          <View
            ...
            <Text style={styles.dogNameText}>
              ...
              {` End at ${sessionData.endDate.getMonth() +
                1}/${sessionData.endDate.getDate()}/${sessionData.endDate.getFullYear()}`}
            </Text>
          </View>
          ...
          <View
            ...
            <TouchableOpacity
              disabled={sessionData.additionalNotes.length <= 35}
              onPress={() => {
              ...
              <Text style={styles.dogNameText}>
                ...
                {sessionData.additionalNotes.length > 35 && !wantReadMore
                  ? sessionData.additionalNotes.substring(0, 35) + '...'
                  : sessionData.additionalNotes}{' '}
              </Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity
            style={[
              styles.addNewInstructionButton,
              {
                alignItems: 'center',
                justifyContent: 'center',
                marginBottom: 15,
            ...
            <Text style={{fontSize: 12, color: '#ffffff'}}>Message</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={{marginVertical: 18, marginRight: 4}}>
        <View
          ...
          <View>
            <Text style={styles.instructionTitle}>Care Instructions</Text>
          </View>

          {isOwner ? (
            <TouchableOpacity
              onPress={() => {
                props.navigation.navigate('AddNewInstruction', {
                  addANewInstruction: addANewInstruction,
                });
              }}
              style={{
                backgroundColor: 'transparent',
                  ...
                  {instruction.reminder ? (
                    <View
                      ...
                      <MaterialCommunityIcon
                        name={'bell-outline'}
                        size={16}
                        color={colors.tagIconGrey}
                      />
                      <Text style={styles.instructionTagText}>
                        Scheduled on{' '}
                        {new Date(
                          instruction.date.seconds * 1000,
                        ).toDateString()}
                      </Text>
                    </View>
                  ) : (
  ...
  render = () => {
    ...
    return (
      <ScrollView style={{padding: 12}}>
        {this.state.todaySessionData.length === 0 ? (
          <Text></Text>
        ) : (
          <IndividualSessionCard
            sessionData={this.state.todaySessionData[0]}
            navigation={this.props.navigation}
            addANewInstruction={this.addANewInstruction}
          />
        )}
      </ScrollView>
    );
