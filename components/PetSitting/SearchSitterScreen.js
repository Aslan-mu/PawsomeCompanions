  ...
  return (
    <View
      style={{
        ...
        paddingHorizontal: 4,
        height: 30,
        backgroundColor: '#a59af5',
        flexDirection: 'row',
        alignItems: 'center',
      }}>
      <Text style={{fontSize: 16, color: '#ffffff'}}> {name} </Text>
      <TouchableOpacity
        underlayColor="#a59af5"
        onPress={() => deleteAName(name, id)}>
...
function Item({
  ...
  if (item.userID === global.currentUser.id) {
    return <View></View>;
  }
  ...
  return (
    <TouchableHighlight onPress={onChecked} underlayColor="white">
      <View style={styles.item}>
        <TouchableHighlight
          onPress={() =>
            navigation.navigate('UserProfilePage', {userData: item})
          }>
          <Image
            source={{uri: item.image}}
            style={styles.personProfilePhoto}></Image>
        </TouchableHighlight>
        <View>
          <Text style={styles.sitterName}>{item.name}</Text>
          <Text style={styles.peopleRecommended}>
            {item.numOfRecommendations} People Recommended
          </Text>
        </View>

        <View style={{marginLeft: 30}}>
          <Text style={styles.miAway}>Midtown West</Text>
        ...
        {checked ? (
          <Icon style={styles.checker} name={'done'} size={24}>
            {' '}
          </Icon>
        ) : (
          <View />
        ...
        {sitterList.map((s, i) => {
          ...
          return (
            <NameTag
              ...
              id={s.id}
              key={s.id + i}
              deleteAName={props.deleteAName}></NameTag>
          );
        })}
      </ScrollView>
    </View>
...
export default class SearchSitterList extends React.Component {
    ...
    headerRight: (
      <Button
        ...
        onPress={() => {
          ...
          navigation.navigate('RequestSentConfirmation');
        }}></Button>
    ),
  });
  ...
  componentDidMount = () => {
    ...
    firebaseSvc.getAllUserData(snapshot => {
      ...
      incomingUserData.sort(
        (a, b) => a.numOfRecommendations - b.numOfRecommendations,
      );
      // console.log(incomingUserData[0].numOfRecommendations)
      this.setState({
        petSittingData: [{title: 'Search Result', data: incomingUserData}],
      });
    });
  };
  render = () => {
    return (
      <View>
        ...
        <SearchBar
          sitterList={this.state.sitterList}
          deleteAName={this.deleteAName}></SearchBar>
        <SectionList
          ...
          renderItem={({item}) => (
            <Item
              ...
              addANewSitterCandidate={this.addANewSitterCandidate}
              deleteAName={this.deleteAName}
              sitterChecked={this.state.sitterList
                .map(s => s.id)
